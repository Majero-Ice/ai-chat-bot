...
import type { CrawlerResult, CrawledPage } from '../../crawler/interfaces/crawler-result.interface';

@Injectable()
export class CrawlerDataStrategy implements ParserStrategy {
  ...
  async parse(data: string): Promise<FileParseResult> {
    ...
    try {
      crawlerResult = JSON.parse(data);
    } catch (error) {
      throw new BadRequestException(`Failed to parse crawler data as JSON: ${error.message}`);
    }

    if (!crawlerResult || !Array.isArray(crawlerResult.pages)) {
      throw new BadRequestException('Invalid crawler result format: pages array is required');
    }

    // Объединяем контент всех страниц с метаданными
    const combinedContent = this.combinePagesContent(crawlerResult.pages);
    ...
    const normalizedText = this.normalizeText(combinedContent);
  ...
  /**
   * Объединяет контент всех страниц с метаданными (URL, заголовок)
   */
  private combinePagesContent(pages: CrawledPage[]): string {
    return pages
      .map((page) => {
        const pageContent = [
